...
//     let timers = []
//     steps.forEach((_, i) => {
//       timers.push(setTimeout(() => setRevealed(i), 600 + i * 1000))
//     })
//     return () => timers.forEach(clearTimeout)
//   }, [])
...
//               initial={{ opacity: 0, y: 10 }}
//               animate={{ opacity: 1, y: 0 }}
//               transition={{ delay: 8.1 + i * 0.07, duration: 0.3 }}
//               className="p-3 rounded-xl text-center"
//               style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}
//             >
//               <div className="w-2 h-2 rounded-full mx-auto mb-1.5" style={{ background: inn.color }} />
...
export default function PipelineSlide() {
  ...
  useEffect(() => {
    let timers = []
    steps.forEach((_, i) => {
      timers.push(setTimeout(() => setRevealed(i), 600 + i * 1800))
    })
    return () => timers.forEach(clearTimeout)
  }, [])
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-4 md:px-8 py-4">
      ...
      <motion.div
        initial={{ opacity: 0, y: 14 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 13.5, duration: 0.5 }}
        className="w-full max-w-6xl"
      >
        <div className="flex items-center gap-2 mb-2.5">
          <div className="h-px flex-1" style={{ background: 'rgba(255,255,255,0.06)' }} />
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
          {innovations.map((inn, i) => (
            <motion.div
              ...
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 13.6 + i * 0.07, duration: 0.3 }}
              className="p-3 rounded-xl text-center"
              style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}
            >
              <div className="w-2 h-2 rounded-full mx-auto mb-1.5" style={{ background: inn.color }} />
